...
//            alert("--" + diskstatus + "--");
        ...
        if (!isNaN(parentid)) {
            if (data.Type === "disk") {
                $("#raid_item" + id + "-" + parentid).append("<div style=\"text-align:center; float:left; margin-bottom:5px; margin-right:10px; margin-left:10px;\" title=\"" + info + "\"><img src=\"phpsysinfo/plugins/raid/gfx/" + img + "\" alt=\"" + alt + "\" style=\"width:60px;height:60px;\" /><br><small>" + data.Name + "</small></div>");
            } else {
                if (parentid === 0) {
